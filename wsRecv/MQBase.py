...
class MQBase(object):
    def __init__(self, host, port, exchange, exchange_type, user, password, virtualhost, queue, ack=True, persist=True):
        ...
        self.param = pika.ConnectionParameters(host=self.host, port=self.port,credentials=self.cerdentials,virtual_host=self.virtual_host)
    ...
    def open_channel(self):
        ...
        if not self.channel:
            ...
            # 声明交换机
            self.exchange.exchange_declare(exchange=self.exchange,exchange_type=self.exchange_type,durable=self.persist)
            # 声明队列
            ...
            # 消息持久化
            self.proprties = pika.BasicProperties(delivery_mode=(2 if self.persist else 0))
