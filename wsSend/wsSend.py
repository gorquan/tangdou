...
def sendMessage(message, recvConn, recvChannel, recvAck_tag, ws, sendCount, log):
    try:
        ...
        while True:
            ws.send(message)
            if msg['action'] in ('set_friend_add_request', 'set_group_add_request'):
                log.info("message send success! the message flag is %s" %
                         msg['params']['flag'])
                ...
                if result['status'] == 'ok' and result['retcode'] == 0:
                    log.info("message send success! the message is: %s, the resulrt is %s" % (
                        message, result))
                    recvConn.add_callback_threadsafe(
                        partial(recvChannel.basic_ack, recvAck_tag))
                    break
                elif result['status'] == 'failed' and result['retcode'] < 0:
                    log.error(
                        "coolq component error, the message is %s, the result is %s now throw array the message" % (message, result))
                    recvConn.add_callback_threadsafe(
                        partial(recvChannel.basic_ack, recvAck_tag))
                    break
                if sendCount <= count:
...
def deal_message(receiver, WSHost, WSPort, WSUri, sendCount, log):
    try:
        ...
        log.info("connect to websocket server: ws://%s:%s%s success" %
                 (WSHost, WSPort, WSUri))
        while True:
            ...
            threading.Thread(target=sendMessage, args=(
                message, recvConn, recvChannel, recvAck_tag, ws, sendCount, log,)).start()
    except Exception as e:
        log.error("connect to websocket server: ws://%s:%s%s failed, the reason is %s" %
                  (WSHost, WSPort, WSUri, e))
        exit(1)
...
if __name__ == "__main__":
    ...
    receiver = MQReceive(MQServerHost=Config['sendRabbitmq']['host'], MQServerPort=Config['sendRabbitmq']['port'], MQServerExchange=Config['sendRabbitmq']['exchange'], MQServerExchange_type=Config['sendRabbitmq']['exchange_type'], MQServerUser=Config['sendRabbitmq']
                         ['user'], MQServerPassword=Config['sendRabbitmq']['password'], MQServerVirtualhost=Config['sendRabbitmq']['virtualhost'], MQServerQueue=Config['sendRabbitmq']['queue'], MQServerPrefetch_count=Config['sendRabbitmq']['prefetch_count'])
